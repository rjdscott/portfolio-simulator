#!/usr/bin/env python3
"""
CLI: Convert price data to Parquet format (Phase 2 storage baseline).
...
from src.data.storage import convert_all, storage_size_report


def main():
    parser = argparse.ArgumentParser(
        description="Convert CSV price data to Parquet format."
    )
    parser.add_argument(
        "--compression", nargs="+",
    ...
    convert_all(compressions=args.compression)
